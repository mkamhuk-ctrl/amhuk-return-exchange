...
import { type ExchangeRow } from "../lib/data";
import { useToast } from "../lib/toast";
import { useSupabaseData } from "../services/useSupabaseData";
import { fetchExchangeRequests, updateExchangeStatus } from "../services/exchangeService";
import { subscribe } from "../services/events";
import { cn } from "../utils/cn";

const searchFields = [
  { value: "orderId", label: "Order ID" },
  { value: "id", label: "Exchange ID" },
...
const stats = [
  ...
  { key: "Refund Completed", label: "Exchange Done", color: "text-emerald-600", dark: "dark:text-emerald-400" },
];

export default function Exchanges() {
  const toast = useToast();
  const { data, status, error, refetch, setData } = useSupabaseData(fetchExchangeRequests, []);
  ...
  // Refetch whenever a new exchange is submitted from the Customer Portal
  useEffect(() => subscribe("exchanges:changed", () => { refetch(); }), [refetch]);
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      const matchesSearch =
        !query || String(r[searchField as keyof ExchangeRow]).toLowerCase().includes(query.toLowerCase());
      return matchesStatus && matchesSearch;
    });
  }, [rows, searchField, query, statusFilter, from, to]);

  const updateRowStatus = (id: string, newStatus: string) => {
    setData(rows.map((r) => (r.id === id ? { ...r, status: newStatus as ExchangeRow["status"] } : r)));
  };

  const handleApprove = async (r: ExchangeRow) => {
    updateRowStatus(r.id, "Approved");
    if (selected?.id === r.id) setSelected({ ...selected, status: "Approved" });
    try {
      await updateExchangeStatus(r.id, "Approved");
  ...
  const columns: Col<ExchangeRow>[] = [
    {
      key: "id",
      label: "Exchange ID",
      render: (r) => (
        <button onClick={() => setSelected(r)} className="font-semibold text-blue-600 hover:underline dark:text-blue-400">
          {r.id}
        </button>
      ),
    },
    {
      ...
      label: "Order ID",
      render: (r) => (
        <button onClick={() => setSelected(r)} className="font-mono text-sm font-semibold text-blue-600 hover:underline dark:text-blue-400">
          {r.orderId}
        </button>
      ),
    },
    {
      ...
      render: (r) => (
        <div className="flex items-center gap-2.5">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-500 to-purple-500 text-[11px] font-bold text-white">
  ...
  const onExport = () => {
    const head = ["Exchange ID", "Order ID", "Customer", "Mobile", "Product", "SKU", "Date", "Time"].join(",");
    const body = filtered
    ...
    a.download = "exchange-requests.csv";
    a.click();
  };
    ...
    <div>
      ...
      {status === "loading" && !data && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-20 dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-3 text-sm font-medium text-slate-500">Loading exchange requests…</p>
        </div>
      )}
        ...
        row={
          selected
            ? {
                ...
                currentSize: selected.currentSize,
                newSize: selected.newSize,
                currentColor: selected.currentColor,
                newColor: selected.newColor,
